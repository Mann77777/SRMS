    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.holidays.update', $holiday->id) }}" method="POST">
                ...
                <div id="date-fields-container" style="{{ $holiday->is_recurring ? 'display:none' : '' }}">
                    <!-- Single day (for holidays and events) -->
                    <div id="single-day-fields" class="form-group" 
                         style="{{ (old('type', $holiday->type) == 'semestral_break' || old('type', $holiday->type) == 'exam_week') ? 'display:none' : '' }}">
                        <label for="date">Date <span class="text-danger">*</span></label>
                        <input type="date" class="form-control @error('date') is-invalid @enderror" 
                               id="date" name="date" value="{{ old('date', $holiday->date ? $holiday->date->format('Y-m-d') : '') }}">
                        @error('date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div id="multi-day-fields" class="form-group"
                         style="{{ (old('type', $holiday->type) == 'semestral_break' || old('type', $holiday->type) == 'exam_week') ? '' : 'display:none' }}">
                ...
                <!-- Recurring fields -->
                <div id="recurring-fields" class="form-group" style="{{ $holiday->is_recurring ? '' : 'display:none' }}">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <select class="form-control @error('recurring_month') is-invalid @enderror" 
                                ...
                                @for ($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}" {{ old('recurring_month', $holiday->recurring_month) == $i ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-control @error('recurring_day') is-invalid @enderror" 
                                ...
                                @for ($i = 1; $i <= 31; $i++)
                                    <option value="{{ $i }}" {{ old('recurring_day', $holiday->recurring_day) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                            @error('recurring_day')
                                <div class="invalid-feedback">{{ $message }}</div>
